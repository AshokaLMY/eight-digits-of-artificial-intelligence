...
int countH(state &q){ //计算启发函数值h=p(n)+3s(n)
    int array[3][3];
    int p=0,s=0,i,j;
    for(i=0;i<3;i++)
        for(j=0;j<3;j++)
        array[i][j]=q.panel[i][j];

    //开始计算s(n)
    i=0;
    for(j=0;j<2;j++)
        if(array[i][j]>array[i][j+1]) s=s+2;
    j=2;
    for(i=0;i<2;i++)
        if(array[i][j]>array[i+1][j]) s=s+2;
    i=2;
    for(j=2;j>0;j--)
        if(array[i][j]>array[i][j-1]) s=s+2;
    if(array[2][0]>array[1][0]) s=s+2;
    if(array[1][1]) s=s+1;
    if(array[0][0]&&array[1][1]) s=s-2;
    ...
    {
        case 0: break;
        case 1:p=fabs(i-0)+fabs(j-0)+p;break;
        case 2:p=fabs(i-0)+fabs(j-1)+p;break;
        case 3:p=fabs(i-0)+fabs(j-2)+p;break;
        case 4:p=fabs(i-1)+fabs(j-2)+p;break;
        case 5:p=fabs(i-2)+fabs(j-2)+p;break;
        case 6:p=fabs(i-2)+fabs(j-1)+p;break;
        case 7:p=fabs(i-2)+fabs(j-0)+p;break;
        case 8:p=fabs(i-1)+fabs(j-0)+p;break;
